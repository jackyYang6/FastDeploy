...
class ModelRegistry:
    ...
    def _try_resolve_paddleformers(
        ...
        # Explicit paddleformers or paddleformers_pure backend requested
        if model_impl == "paddleformers":
            backend_arch = "PaddleFormersForCausalLM"
        elif model_impl == "paddleformers_pure":
            backend_arch = "PaddleFormersPureForCausalLM"
        # Auto mode fallback when no native implementation exists
        elif model_impl == "auto" and is_fallback:
            backend_arch = "PaddleFormersForCausalLM"
        # fastdeploy explicitly requested - no fallback
        ...
        if not architectures:
            if model_impl in ("paddleformers", "paddleformers_pure"):
                raise ValueError(
